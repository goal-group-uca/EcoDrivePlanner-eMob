...
class TPXCrossover(Crossover[BinarySolution, BinarySolution]):

    def __init__(self, probability: float):
        super(TPXCrossover, self).__init__(probability=probability)
    ...
    def execute(self, parents: List[BinarySolution]) -> List[BinarySolution]:
        ...
        if rand <= self.probability:
            ...
            # print(f"Con un total de {total_number_of_variables}")
...
class IntegerTPXCrossoverDouble(Crossover[IntegerSolution, IntegerSolution]):

    def __init__(self, probability: float):
        super(IntegerTPXCrossoverDouble, self).__init__(probability=probability)
    ...
    def execute(self, parents: List[IntegerSolution]) -> List[IntegerSolution]:
        ...
        if rand <= self.probability:
            ...
            # 2. Calculate the points to make the crossover
            variable_to_cut1 = random.randrange(0, 100)
            variable_to_cut2 = random.randrange(0, 100)

            points = [variable_to_cut1, variable_to_cut2]
            points.sort()

            # 3. Apply the crossover to the variable (First Part)
            for i in range(variable_to_cut1 + 1, variable_to_cut2 + 1):
                swap = offspring[0].variables[i]
                offspring[0].variables[i] = offspring[1].variables[i]
                offspring[1].variables[i] = swap
            ...
            # 2. Calculate the points to make the crossover
            variable_to_cut1 = random.randrange(100 + 1, total_number_of_variables)
            variable_to_cut2 = random.randrange(100 + 1, total_number_of_variables)

            points = [variable_to_cut1, variable_to_cut2]
            points.sort()

            # 4. Apply the crossover to the variable (Second Part)
            for i in range(variable_to_cut1 + 1, variable_to_cut2 + 1):
                swap = offspring[0].variables[i]
                offspring[0].variables[i] = offspring[1].variables[i]
                offspring[1].variables[i] = swap
    ...
    def get_name(self) -> str:
        return 'Integer Two point crossover (Double solution)'
